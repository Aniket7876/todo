  ...
  const handleSaveTask = async (taskData: Partial<Task>) => {
    ...
    try {
      if (selectedTask) {
        ...
        const updatedTask: Task = await response.json();
        setTasks((current) => current.map((taskItem) => (taskItem.id === updatedTask.id ? updatedTask : taskItem)));
      } else {
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-10">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <span
                  ...
                  style={{
                    background:
                      "linear-gradient(120deg, rgba(114,109,255,0.45), rgba(255,106,213,0.45), rgba(56,189,248,0.4))",
                  }}
                />
        ...
        <section className="relative z-10 grid gap-4 md:auto-rows-fr md:grid-cols-2 xl:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.id}
              className="group relative flex h-full flex-col justify-between overflow-hidden rounded-[26px] border border-white/10 bg-white/5 p-6 shadow-[0_24px_40px_-28px_rgba(15,23,42,0.9)] backdrop-blur-xl"
            >
              <span
        ...
        <div className="relative z-10 flex snap-x items-stretch gap-6 overflow-x-auto pb-8 pt-2 lg:auto-rows-fr lg:grid lg:snap-none lg:grid-cols-3 lg:gap-8 lg:overflow-visible lg:pb-12">
          {isLoading ? (
