    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.warn("Failed to parse signup error", parseError);
        }
        setError(message);
        return;
      }
      ...
      console.error("Failed to submit signup form", submissionError);
  ...
  return (
    <div className="relative z-10 w-full max-w-lg overflow-hidden rounded-[32px] border border-white/10 bg-white/10 p-10 shadow-[0_30px_60px_-35px_rgba(15,23,42,0.95)] backdrop-blur-xl sm:max-w-md sm:p-12">
      <span
        ...
        style={{ background: "linear-gradient(135deg, rgba(114,109,255,0.35), rgba(255,106,213,0.25), rgba(56,189,248,0.22))" }}
      />
      ...
      <div className="relative space-y-8">
        <header className="space-y-2 text-center">
          ...
          <p className="text-sm text-white/70">Bring your tasks to life with a personal workspace.</p>
        </header>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            <label htmlFor="username" className="text-xs font-semibold uppercase tracking-[0.24em] text-white/50">
              Username
            </label>
            <input
              id="username"
              ...
              placeholder="Choose how we should greet you"
            />
          </div>

          <div className="space-y-2">
            <label htmlFor="email" className="text-xs font-semibold uppercase tracking-[0.24em] text-white/50">
              Email
            </label>
            <input
              id="email"
              ...
              onChange={handleChange}
              required
              className="w-full rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm text-white placeholder-white/40 shadow-inner focus:border-white/35 focus:outline-none focus:ring-2 focus:ring-sky-400/45"
        ...
        <p className="text-center text-xs text-white/65">
          ...
          <Link href="/login" className="font-semibold text-sky-200 underline-offset-4 hover:underline">
            Sign in
          </Link>
        </p>
      </div>
